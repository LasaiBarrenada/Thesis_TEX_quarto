...
\newglossaryentry{net_benefit}{
  ...
  description={A decision-analytic measure that combines true positives and false positives using a chosen decision threshold. Mathematically defined as: $\frac{\text{TP}}{N} - \frac{\text{FP}}{N} \times \frac{\text{threshold}}{1 - \text{threshold}}$, where TP is the number of true positives, FP is the number of false positives, and N is the total number of patients.}
}
...
\newglossaryentry{random_effects_meta_analysis}{
  ...
  description={A meta-analytic framework that allows true effects or performance measures to vary across clusters.}
}
...
\newglossaryentry{properness}{
  ...
  text={proper},
  first={\textbf{proper}},
  description={A property of a performance measure that is optimized when the true model is used, which means that the true probabilities are used. If it is only optimized with the true model, it is said to be strictly proper. Instead if it is optimized by the true model but also by other models, it is said to be semi-proper. Improper measures can be higher for models that are not the true model.}
}
...
\newglossaryentry{observed_expected_ratio}{
  ...
  description={A calibration measure comparing the total number of observed events to the total number of expected events from the model. A value of 1 indicates perfect agreement in the mean. Values less than 1 indicate overestimation of risk, while values greater than 1 indicate underestimation of risk. Analogous to the calibration intercept.}
}
...
\newglossaryentry{hyperparameter_tuning}{
  ...
  description={The data driven process of selecting the optimal values for hyperparameters in a machine learning model or AI algorithm, often using resampling methods such as cross-validation to evaluate performance across different hyperparameter combinations.}
}
...
\newglossaryentry{apparent_performance}{
  ...
  description={Model performance assessed in the exact same data (i.e. training data) used for model development.}
}
\newglossaryentry{binned calibration}{
  name={Binned calibration},
  text={binned calibration},
  first={\textbf{binned calibration}},
  plural={binned calibrations},
  firstplural={\textbf{binned calibrations}},
  description={A method for assessing calibration by binning the data into groups based on predicted probabilities and plotting observed outcome proportions against mean predicted probabilities within each bin.}
}