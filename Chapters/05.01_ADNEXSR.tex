\chapter[ADNEX model performance systematic review]{The ADNEX risk prediction model for ovarian cancer diagnosis: A systematic review and meta-analysis of external
validation studies}
% Only include if the thesis is written as a collection of papers
%_________________________
\includegraphics*[width=0.2\linewidth]{logos/BMJ.png}\vspace{0.5cm}
...
published as: 
\begin{quote}
 Barreñada L, Ledger A, Dhiman P, Collins G, Wynants L, Verbakel JY, et al. \gls{ADNEX}
risk prediction model for diagnosis of ovarian cancer: systematic review and
meta-analysis of external validation studies. BMJ Medicine. 2024;3:e000817.
\end{quote}
%_________________________
\section*{Abstract}
\textbf{Objectives:} To conduct a systematic review of studies externally
validating the \gls{ADNEX} model for ovarian cancer diagnosis and perform a
meta-analysis of its performance.

\textbf{Design:} Systematic review, meta-analysis.

\textbf{Data sources:} Medline, EMBASE, WOS, Scopus, and EuropePMC up to
15/05/2023.

\textbf{Review methods:} We included external validation studies of the
performance of \gls{ADNEX} using any study design and any study population comprising
patients with an adnexal mass. Two independent reviewers extracted data.
Disagreements were resolved through discussion. Reporting quality of the studies
was scored using the \gls{TRIPOD} reporting guideline and methodological conduct and
risk of bias using the \gls{PROBAST} tool. We performed random effects meta-analysis
of the \gls{AUC}, sensitivity and specificity at the 10\% risk of malignancy
threshold, and Net Benefit and Relative Utility at the 10\% risk of malignancy
threshold.

\textbf{Results:} We included 47 studies (17,007 tumours) with median study
sample size 261 (range 24-4905). On average, 61\% of \gls{TRIPOD} items were reported.
Handling of missing data, sample size justification, and model calibration were
rarely described. 91\% of validations were at high risk of bias, mainly due to
the unexplained exclusion of incomplete cases, low sample size, or absent
calibration assessment. The summary \gls{AUC} to distinguish benign from malignant
tumours in operated patients was 0.93 (95\% \gls{CI} 0.92-0.94, 95\% prediction
interval 0.85-0.98) for \gls{ADNEX} with CA125 as a predictor (9202 tumours, 43
centres, 18 countries, 21 studies) and 0.93 (95\% \gls{CI} 0.91-0.94, 95\% prediction
interval 0.85-0.98) for \gls{ADNEX} without CA125 (6309 tumours, 31 centres, 13
countries, 12 studies). The estimated probability that the model has clinical
utility in a new centre was 95\% (with CA125) and 91\% (without CA125). When
restricting analysis to studies at low risk of bias, summary \glspl{AUC} were 0.93
(with CA125) and 0.91 (without CA125), and estimated probabilities that the
model has clinical utility were 89\% (with CA125) and 87\% (without CA125).

\textbf{Discussion:} \gls{ADNEX} performed well to distinguish benign from malignant
tumours in populations from different countries and settings regardless of
whether CA125 was used or not. A key limitation is that calibration was rarely
assessed.

\textbf{Review registration:} PROSPERO, CRD42022373182
...
Supplementary Material S4-6 provides details on the statistical methodology,
...
Europe & 18 (38) & Common countries: Poland (7), Italy (4), Spain (4), UK (3), Sweden (3) \\
South America & 3 (6) & \\
North America & 2 (4) & \\
...
Unclear & 6 (13) & \\
\textbf{ADNEX version} & & \\
\gls{ADNEX} with CA125 & 37 (79) & 21 only used \gls{ADNEX} with CA125, 16 used both \\
\gls{ADNEX} without CA125 & 19 (40) & 3 only used \gls{ADNEX} without CA125, 16 used both \\
Mixed & 3 (6) & \gls{ADNEX} with CA125 used if CA125 was available \\
Unclear & 4 (9) & \\
...
Surgically and non-surgically managed patients & 4 (9) & \\
\bottomrule
\end{tabularx}
\end{table}
...
\caption{Risk of bias overall and by subdomain in 63 validations, assessed by PROBAST (Prediction model Risk Of Bias Assessment Tool).}
\label{fig:1:3}
\end{figure}
...
\label{tab:2}
\footnotesize
\begin{tabularx}{\textheight}{Xp{2cm}p{3.2cm}p{2.5cm}p{1cm}}
\toprule
\textbf{Meta-analysis} & \textbf{Studies (centres)} & \textbf{Summary estimate (95\% \gls{CI})} & \textbf{95\% PI} & $\tau^2$ \\
\midrule
\textbf{Main analysis} & & & & \\
...
Suplelmentary material is available online \cite{barrenadaADNEXRiskPrediction2024}.
The data and code used for the analysis and the generation of the figures is
publicly available in Open Science Framework (https://osf.io/jtsvd/)
