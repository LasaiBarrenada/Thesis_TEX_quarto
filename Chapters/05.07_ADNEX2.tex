...
as:
\begin{quote}
\textbf{Barreñada L} et al. ADNEX2: Updating the ADNEX model to predict ovarian cancer for surgically and conservatively managed patients with an adnexal mass. \textit{In preparation}.
\end{quote}

\begin{quote}
\textbf{Supplementary material and code} available at \url{https://osf.io/36n8m/overview?view_only=0f1fa6d9a5e34e4d901c05922d0d0307}.
\end{quote}
\vspace{-0.5em}
...
\noindent\textbf{Objective:} The \gls{ADNEX} prediction model to diagnose ovarian cancer has shown good performance in many external validation studies. Our aim is to update \gls{ADNEX} to ADNEX2, using recent data that also include patients that were managed with follow-up. ADNEX2 is designed for use after applying the \glspl{BD}, a set of ultrasound profiles with a risk of malignancy lower than 1\%.
...
\noindent\textbf{Results:} 6847 patients were included; 3900 (57\%) were immediately operated, 2639 (39\%) were managed with clinical and ultrasound follow-up, and for 308 (4\%) the management was unknown. The \glspl{BD} applied in 2190 (32\%) patients, of whom 10 (0.5\%, 95\% \gls{CI} 0.2--0.8) had a histologically confirmed malignancy. ADNEX2 was developed on the remaining 4657 patients. On all 6847 patients, the \gls{AUC} was 0.95 (95\% \gls{CI} 0.93--0.96) for \glspl{BD} followed by ADNEX2 including \gls{CA125} if available. On 4657 patients for which \glspl{BD} did not apply, ADNEX2 including \gls{CA125} if available had an \gls{AUC} of 0.93 (0.91--0.94).

\noindent\textbf{Conclusions:} ADNEX2 is the first model in this field that is developed on patients irrespective of subsequent management. ADNEX2 targets patients for which the \glspl{BD} do not apply. For the decision of whom to refer for surgery, \gls{CA125} is not crucial. However, \gls{CA125} is strongly recommended to preoperatively assess the likely type of malignancy.
...
We compared the available sample size with Pate et al.'s sample size criteria for multinomial logistic regression prediction models \cite{pateMinimumSampleSize2023} (details in Supplementary material S2.1). We followed a prespecified statistical analysis plan for this study preregistered in \gls{OSF}.
...
Researchers that aim to externally validate the model can do it by using evaluatr R package (in development). This package provides a safe environment for model developers and model evaluators where the developer does not have access to the patient data, and the evaluator does not have access to the model coefficients but can obtain a vector of predicted risks to evaluate the model's aggregate performance. Access to the model is given case by case, upon reasonable request to the model developers.
...
Predefined subgroups were based on menopausal status (premenopausal vs postmenopausal), type of centre (oncology vs non-oncology) and a subgroup defined as patients that were operated within 120~days after inclusion scan. The \glsentryshort{AUC}s of the two step strategy with ADNEX2 or \gls{ADNEX} ranged from 0.91 to 0.95. \glsentryshort{AUC}s were higher in postmenopausal than in premenopausal patients and in oncology than in non-oncology centres. Pairwise \glsentryshort{AUC}s in the subgroup of patients operated within 120~days showed similar performance to the performance in the whole population. The discrimination of the two step strategy with ADNEX2 or \gls{ADNEX} showed identical results. Predicted risks where underestimated across all subgroups, with slightly better calibration in non-oncology centres. Calibration was more heterogeneous between centres among oncology centres than among non-oncology centres, as indicated by the wider prediction intervals. Calibration was better but slightly more heterogeneous for ADNEX2 than for \gls{ADNEX} across all subgroups. The number of cases in the subgroups was too low to perform multinomial calibration.

For the 10\% threshold, sensitivity ranged between 0.85 and 0.96 and specificity between 0.72 and 0.89. For the 1\% threshold, the sensitivity ranged between 0.91 and 0.99 and specificity between 0.31 and 0.61. The two-step strategy with ADNEX2 showed clinical utility across the whole range of decision thresholds from $>$1\% to 40\% in all subgroups, with comparable performance irrespective if \gls{CA125} was used or not. However, at the 1\% decision threshold the utility was lower than the strategy of treating all patients. The proportion of centres in which the two-step strategy has clinical utility varied between 45\% and 84\% for the 1\% decision threshold and between 89\% and 97\% for the 10\% decision threshold. Clinical utility was higher in postmenopausal patients than premenopausal patients and in oncology centres rather than non-oncology centres. Clinical utility of the two-step strategy with \gls{ADNEX} was similar to with ADNEX2 across all subgroups.
...
The \gls{IOTA}5 protocol was registered in clinicaltrials.gov (https://clinicaltrials.gov/study/NCT01698632) and the statistical analysis plan was preregistered in an Open Science Foundation (\gls{OSF}) preregistration.
