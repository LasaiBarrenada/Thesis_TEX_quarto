...
published as: 
\begin{quote}
Barreñada L, Dhiman P, Timmerman D, Boulesteix AL \& Van Calster B. Understanding overfitting in random forest for probability estimation: a visualization and simulation study. Diagn Progn Res 8, 14 (2024). https://doi.org/10.1186/s41512-024-00177-1
\end{quote}
%_________________________
\section*{Abstract}
\textbf{Background:} Random forests have become popular for clinical risk
prediction modeling. In a case study on predicting ovarian malignancy, we
observed training \glspl{AUC} close to 1. Although this suggests overfitting,
performance was competitive on test data. We aimed to understand the behavior of
random forests for probability estimation by (1) visualizing data space in three
real-world case studies and (2) a simulation study.

\textbf{Methods:} For the case studies, multinomial risk estimates were
visualized using heatmaps in a 2-dimensional subspace. The simulation study
included 48 logistic data-generating mechanisms (\gls{DGM}), varying the predictor
distribution, the number of predictors, the correlation between predictors, the
true \gls{AUC}, and the strength of true predictors. For each \gls{DGM}, 1000 training
datasets of size 200 or 4000 with binary outcomes were simulated, and random
forest models were trained with minimum node size 2 or 20 using the ranger R
package, resulting in 192 scenarios in total. Model performance was evaluated on
large test datasets ($N = 100{,}000$).

\textbf{Results:} The visualizations suggested that the model learned ``spikes
of probability'' around events in the training set. A cluster of events created
a bigger peak or plateau (signal), isolated events local peaks (noise). In the
simulation study, median training \glspl{AUC} were between 0.97 and 1 unless there were
4 binary predictors or 16 binary predictors with a minimum node size of 20. The
median discrimination loss, i.e., the difference between the median test \gls{AUC} and
the true \gls{AUC}, was 0.025 (range 0.00 to 0.13). Median training \glspl{AUC} had Spearman
correlations of around 0.70 with discrimination loss. Median test \glspl{AUC} were
higher with higher events per variable, higher minimum node size, and binary
predictors. Median training calibration slopes were always above 1 and were not
correlated with median test slopes across scenarios (Spearman correlation
$-0.11$). Median test slopes were higher with higher true \gls{AUC}, higher minimum
node size, and higher sample size.

\textbf{Conclusions:} Random forests learn local probability peaks that often
yield near perfect training \glspl{AUC} without strongly affecting \glspl{AUC} on test data.
...
would lead to reduced performance on new data
 \cite{hastieElementsStatisticalLearning2001,steyerbergClinicalPredictionModels2019a}.
...
The paper outline is as follows. In the ``Random forest for probability
estimation'' section, we summarize the \gls{RF} algorithm for probability estimation,
in the ``Case studies'' section, we visualize the predictions for the ovarian
tumor data, and present two additional case studies. In the ``Simulation study''
section, we present a simulation study to explore the effect of tree depth and
training sample size and the data generation mechanism (\gls{DGM}) to better
understand the behavior of the \gls{RF} algorithm. In the ``Overall discussion''
...
In \gls{MLR} models, we modeled continuous predictors using restricted cubic splines
...
years) and CA125 (in IU/L) as continuous variables, and five ultrasound based
...
available in the \gls{OSF} repository (\url{https://osf.io/y5tqv/}).
...
Repository, \url{https://osf.io/y5tqv/}). However, our aim was to understand
...
in the \gls{OSF} repository (\url{https://osf.io/y5tqv/})
