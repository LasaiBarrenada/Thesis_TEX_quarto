...
published as: 
\begin{quote}
\textbf{Barreñada L}, Dhiman P, Timmerman D, Boulesteix AL \& Van Calster B. Understanding overfitting in random forest for probability estimation: a visualization and simulation study. \textit{Diagn Progn Res}. 2024;8(1):14. \url{https://doi.org/10.1186/s41512-024-00177-1}
\end{quote}
...
The \gls{RF} models were trained with \texttt{ranger} package, with $\textit{ntree} = 500$,
...
\begin{figure}
\centering
\includegraphics[width=0.7\textwidth]{figures/RFOverfitting/F3.pdf}
...
for \gls{RF} models we used values for \textit{min.node.size} of 2 or 20 and \texttt{ranger}
...
the \textit{trainControl()} and \textit{train()} functions from the caret R package
...
the \texttt{tuneRanger} R package with 200 iterations for our case studies based on
...
\texttt{randomForest} package or \texttt{scikit-learn}). We think that the current study sheds
...
as cforest from the \texttt{partykit} package
